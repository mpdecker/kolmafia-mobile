...
- Phase plans live under `docs/superpowers/plans/`; ASH batches in `GameRuntimeLibrary.AshP*Batch.kt` — AshP8–P18 registration floor, AshP19–AshP34 live behavioral batches (entity `to_*` resolvers, `$entity[field]` bracket reads via `*EntityFields.kt` including AshP34 `MonsterEntityFields.kt`, modifier lookups via `modifiers/*`; roadmap: `2026-06-11-ash-script-full-parity.md`).
...
- Incremental parity phases use Quest/CLI/Maximizer tracks; Phases 53–76 advanced Tier 1 (AshP19–AshP34 entity batches including MONSTER brackets, CLI long-tail including `journey`/`witchess`/`volcano`, charpane sync for Ed/VYKEA/pasta thrall, tower/hedge maze/PirateRealm sync); `GameRuntimeLibrary.REVISION` uses `phaseNN` format; update `docs/parity-audit.md` after each phase.
