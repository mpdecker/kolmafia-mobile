...
class HiddenCityHandlersTest {
    ...
    private fun decide(choiceId: Int, preference: Int = 0,
                       charState: CharacterState = CharacterState(),
                       effects: List<EffectData> = emptyList()) =
        HiddenCityHandlers.handlers[choiceId]?.decide(ctx(choiceId, preference, charState, effects))

    // Case 780
    @Test fun case780_pref1_apartmentDone_returns6() {
        prefs.setInt("hiddenApartmentProgress", 7)
        assertEquals(6, decide(780, 1))
    }
    @Test fun case780_pref1_cursed_returns1() {
        prefs.setInt("hiddenApartmentProgress", 3)
        val eff = listOf(EffectData(0, EffectPool.CURSE3_EFFECT, 5))
        assertEquals(1, decide(780, 1, effects = eff))
    }
    @Test fun case780_pref1_noCurse_returns2() {
        prefs.setInt("hiddenApartmentProgress", 3)
        assertEquals(2, decide(780, 1))
    }
    @Test fun case780_pref3_lawyerRelocated_returns6() {
        val char = CharacterState(ascensionNumber = 5)
        prefs.setInt("relocatePygmyLawyer", 5)
        assertEquals(6, decide(780, 3, charState = char))
    }
    @Test fun case780_pref3_lawyerNotRelocated_returns3() {
        val char = CharacterState(ascensionNumber = 5)
        prefs.setInt("relocatePygmyLawyer", 4)
        assertEquals(3, decide(780, 3, charState = char))
    }
    @Test fun case780_pref2_returnsPreference() = assertEquals(2, decide(780, 2))

    // Case 781
    @Test fun case781_pref1_progress7_returns2() {
        prefs.setInt("hiddenApartmentProgress", 7); assertEquals(2, decide(781, 1))
    }
    @Test fun case781_pref1_progress0_returns1() {
        prefs.setInt("hiddenApartmentProgress", 0); assertEquals(1, decide(781, 1))
    }
    @Test fun case781_pref1_progressMid_returns6() {
        prefs.setInt("hiddenApartmentProgress", 3); assertEquals(6, decide(781, 1))
    }
    @Test fun case781_pref2_returnsPreference() = assertEquals(2, decide(781, 2))

    // Case 783
    @Test fun case783_pref1_progress7_returns2() {
        prefs.setInt("hiddenHospitalProgress", 7); assertEquals(2, decide(783, 1))
    }
    @Test fun case783_pref1_progress0_returns1() {
        prefs.setInt("hiddenHospitalProgress", 0); assertEquals(1, decide(783, 1))
    }
    @Test fun case783_pref1_progressMid_returns6() {
        prefs.setInt("hiddenHospitalProgress", 4); assertEquals(6, decide(783, 1))
    }

    // Case 785
    @Test fun case785_pref1_progress7_returns2() {
        prefs.setInt("hiddenOfficeProgress", 7); assertEquals(2, decide(785, 1))
    }
    @Test fun case785_pref1_progress0_returns1() {
        prefs.setInt("hiddenOfficeProgress", 0); assertEquals(1, decide(785, 1))
    }

    // Case 787
    @Test fun case787_pref1_progress7_returns2() {
        prefs.setInt("hiddenBowlingAlleyProgress", 7); assertEquals(2, decide(787, 1))
    }
    @Test fun case787_pref1_progress0_returns1() {
        prefs.setInt("hiddenBowlingAlleyProgress", 0); assertEquals(1, decide(787, 1))
    }

    // Case 789
    @Test fun case789_pref2_janitorRelocated_returns1() {
        val char = CharacterState(ascensionNumber = 3)
        prefs.setInt("relocatePygmyJanitor", 3)
        assertEquals(1, decide(789, 2, charState = char))
    }
    @Test fun case789_pref2_janitorNotRelocated_returnsPref() {
        val char = CharacterState(ascensionNumber = 3)
        prefs.setInt("relocatePygmyJanitor", 2)
        assertEquals(2, decide(789, 2, charState = char))
    }
