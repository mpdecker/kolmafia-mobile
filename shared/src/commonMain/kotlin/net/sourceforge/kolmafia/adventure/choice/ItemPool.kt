package net.sourceforge.kolmafia.adventure.choice

object ItemPool {
    // Woods items (IDs 1–12): items found via Three-Tined Fork and Footprints
    val WOODS_ITEM_IDS: IntArray = IntArray(12) { it + 1 }

    const val SPOOKY_SAPLING            = 75
    const val TREE_HOLED_COIN           = 4676
    const val INEXPLICABLY_GLOWING_ROCK = 1121
    const val SPOOKY_GLOVE              = 1125
    const val PAPAYA                    = 498
    const val VALUABLE_TRINKET          = 139
    const val MODEL_AIRSHIP             = 6299
    const val MERKIN_PRESSUREGLOBE      = 3675
    const val SEED_PACKET               = 3553
    const val GREEN_SLIME               = 3554
    const val AZAZEL_OBJECT_1           = 2566
    const val AZAZEL_OBJECT_2           = 2567
    const val AZAZEL_OBJECT_3           = 2568
    const val EXPRESS_CARD              = 1687
    const val PICKOMATIC_LOCKPICKS      = 280
    const val BORIS_KEY                 = 282
    const val JARLSBERG_KEY             = 283
    const val SNEAKY_PETE_KEY           = 284
    const val SKELETON_KEY              = 642
    const val STAR_KEY                  = 665
    const val DIGITAL_KEY               = 691
    const val CLOWN_CAR_KEY             = 10547
    const val BATTING_CAGE_KEY          = 10548
    const val AQUI                      = 10549
    const val KNOB_LABINET_KEY          = 10550
    const val WEREMOOSE_KEY             = 10551
    const val PEG_KEY                   = 10552
    const val KEKEKEY                   = 10553
    const val RABBITS_FOOT_KEY          = 10554
    const val KNOB_SHAFT_SKATE_KEY      = 10555
    const val ICE_KEY                   = 10556
    const val ANCHOVY_CAN_KEY           = 10557
    const val CACTUS_KEY                = 10558
    const val F_C_LE_SH_C_LE_K_Y        = 10559
    const val TREASURE_CHEST_KEY        = 10560
    const val DEMONIC_KEY               = 10561
    const val KEY_SAUSAGE               = 10562
    const val KNOB_TREASURY_KEY         = 10563
    const val SCRAP_METAL_KEY           = 10564
    const val BLACK_ROSE_KEY            = 10565
    const val MUSIC_BOX_KEY             = 10566
    const val ACTUAL_SKELETON_KEY       = 10567
    const val DEEP_FRIED_KEY            = 10568
    const val DISCARDED_BIKE_LOCK_KEY   = 10569
    const val RUFUS_SHADOW_LODESTONE    = 11176
    const val SILVER_SHOTGUN_SHELL      = 5310
    const val CHAINSAW_CHAIN            = 5309
    const val FUNHOUSE_MIRROR           = 5311
    const val MAXWELL_HAMMER            = 6257
    const val TONGUE_BRACELET           = 6258
    const val SILVER_CHEESE_SLICER      = 6259
    const val SILVER_SHRIMP_FORK        = 6260
    const val SILVER_PATE_KNIFE         = 6261
    const val LOLLIPOP_STICK            = 5380
    const val MCCLUSKY_FILE             = 6689
    const val MCCLUSKY_FILE_PAGE5       = 7039
    const val BINDER_CLIP               = 7040
    const val STONE_TRIANGLE            = 7041
    const val GOLD_1970                 = 8424  // "1,970 carat gold"
    const val NEW_AGE_HEALING_CRYSTAL   = 8425
    const val EMPTY_LAVA_BOTTLE         = 8440
    const val VISCOUS_LAVA_GLOBS        = 8442
    const val GLOWING_NEW_AGE_CRYSTAL   = 8455
    const val CRIMBO_CRYSTAL_SHARDS     = 11066
    const val CRYSTAL_CRIMBO_GOBLET     = 9201
    const val CRYSTAL_CRIMBO_PLATTER    = 9202
    const val BURNT_STICK                = 10311
    const val CAMPFIRE_SMOKE             = 10313
    const val GOVERNMENT_FOOD_SHIPMENT   = 10685
    const val GOVERNMENT_BOOZE_SHIPMENT  = 10686
    const val GOVERNMENT_CANDY_SHIPMENT  = 10687
    const val GREY_GOO_RING              = 10602
    const val GOOIFIED_ANIMAL_MATTER     = 10844
    const val GOOIFIED_VEGETABLE_MATTER  = 10845
    const val GOOIFIED_MINERAL_MATTER    = 10846
    const val LOATHING_IDOL_MICROPHONE   = 11263
    const val LOATHING_IDOL_MICROPHONE_75 = 11277
    const val LOATHING_IDOL_MICROPHONE_50 = 11278
    const val LOATHING_IDOL_MICROPHONE_25 = 11279
    const val MIMIC_EGG                  = 11542

    // Chintzy items from Mistress of the Hallowed Halls
    const val CHINTZY_SEAL_PENDANT      = 1941
    const val CHINTZY_TURTLE_BROOCH     = 1942
    const val CHINTZY_NOODLE_RING       = 1943
    const val CHINTZY_SAUCEPAN_EARRING  = 1944
    const val CHINTZY_DISCO_BALL_PENDANT = 1945
    const val CHINTZY_ACCORDION_PIN     = 1946
    const val ANTIQUE_HAND_MIRROR       = 2092

    val MISTRESS_ITEM_IDS: IntArray = intArrayOf(
        CHINTZY_SEAL_PENDANT, CHINTZY_TURTLE_BROOCH, CHINTZY_NOODLE_RING,
        CHINTZY_SAUCEPAN_EARRING, CHINTZY_DISCO_BALL_PENDANT, CHINTZY_ACCORDION_PIN,
        ANTIQUE_HAND_MIRROR,
    )

    const val WINDICLE = 10209

    // ResultProcessor.gainItem high-traffic ItemPool (Phases 1791–1850)
    const val BITCHIN_MEATCAR = 134
    const val DINGY_DINGHY = 141
    const val MOSQUITO_LARVA = 275
    const val RUSTY_SCREWDRIVER = 454
    const val CANDLES = 480
    const val TISSUE_PAPER_IMMATERIA = 605
    const val TIN_FOIL_IMMATERIA = 606
    const val GAUZE_IMMATERIA = 607
    const val PLASTIC_WRAP_IMMATERIA = 608
    const val SOCK = 609
    const val OYSTER_BASKET = 1080
    const val SPOOKY_BICYCLE_CHAIN = 1137
    const val MACGUFFIN_DIARY = 2044
    const val BROKEN_WINGS = 2050
    const val SUNKEN_EYES = 2051
    const val REASSEMBLED_BLACKBIRD = 2052
    const val FORGED_ID_DOCUMENTS = 2064
    const val GMOB_POLLEN = 2096
    const val PHOTOGRAPH_OF_GOD = 2259
    const val WET_STUNT_NUT_STEW = 2266
    const val MEGA_GEM = 2267
    const val CONFETTI = 2329
    const val HOLY_MACGUFFIN = 2334
    const val MOLYBDENUM_MAGNET = 2497
    const val MOLYBDENUM_HAMMER = 2498
    const val MOLYBDENUM_SCREWDRIVER = 2499
    const val MOLYBDENUM_PLIERS = 2500
    const val MOLYBDENUM_WRENCH = 2501
    const val PIRATE_FLEDGES = 3033
    const val POWER_SPHERE = 3049
    const val MARSHMALLOW = 3128
    const val ROASTED_MARSHMALLOW = 3129
    const val DRONE = 3157
    const val BROKEN_DRONE = 3165
    const val REPAIRED_DRONE = 3166
    const val AUGMENTED_DRONE = 3167
    const val TRAPEZOID = 3198
    const val OVERCHARGED_POWER_SPHERE = 3215
    const val VOLCANO_MAP = 3291
    const val STICKER_SWORD = 3508
    const val STICKER_CROSSBOW = 3526
    const val AGUA_DE_VIDA = 4001
    const val GG_TOKEN = 4621
    const val GG_TICKET = 4622
    const val PUMPKIN_CARRIAGE = 4769
    const val DESERT_BUS_PASS = 4770
    const val HOLIDAY_FUN_BOOK = 4810
    const val ANTAGONISTIC_SNOWMAN_KIT = 4812
    const val RECONSTITUTED_CROW = 5117
    const val BIRD_BRAIN = 5118
    const val BUSTED_WINGS = 5119
    const val EMU_JOYSTICK = 5135
    const val EMU_ROCKET = 5136
    const val EMU_HELMET = 5137
    const val EMU_HARNESS = 5138
    const val EMU_UNIT = 5143
    const val SPOOKY_LITTLE_GIRL = 5165
    const val RONALD_SHELTER_MAP = 5171
    const val GRIMACE_SHELTER_MAP = 5172
    const val GROOSE_GREASE = 5379
    const val DEVILISH_FOLIO = 5444
    const val FURIOUS_STONE = 5448
    const val VANITY_STONE = 5449
    const val LECHEROUS_STONE = 5450
    const val JEALOUSY_STONE = 5451
    const val AVARICE_STONE = 5452
    const val GLUTTONOUS_STONE = 5453
    const val SKIFF = 5885
    const val UNCONSCIOUS_COLLECTIVE_DREAM_JAR = 5897
    const val JUNK_JUNK = 6735
    const val TIN_LIZZIE = 6775
    const val FIRST_PIZZA = 7179
    const val LACROSSE_STICK = 7180
    const val EYE_OF_THE_STARS = 7181
    const val STANKARA_STONE = 7182
    const val MURPHYS_FLAG = 7183
    const val SHIELD_OF_BROOK = 7184
    const val PHOTOGRAPH_OF_DOG = 7263
    const val PHOTOGRAPH_OF_RED_NUGGET = 7264
    const val PHOTOGRAPH_OF_OSTRICH_EGG = 7265
    const val PALINDROME_BOOK_2 = 7270
    const val TACO_DAN_RECEIPT = 7428
    const val MORTAR_DISSOLVING_RECIPE = 7495
    const val BLACK_BARTS_BOOTY = 7732
    const val ED_DIARY = 7960
    const val SPELUNKER_FORTUNE = 8073
    const val MAP_TO_KOKOMO = 8182
    const val ESSENCE_OF_BEAR = 8277
    const val YELLOW_SUBMARINE = 8376
    const val MANUAL_OF_NUMBEROLOGY = 8488
    const val TRAINING_BELT = 8699
    const val TRAINING_LEGWARMERS = 8700
    const val TRAINING_HELMET = 8701
    const val SCROLL_SHATTERING_PUNCH = 8702
    const val SCROLL_SNOKEBOMB = 8703
    const val SCROLL_SHIVERING_MONKEY = 8704
    const val ABSTRACTION_ACTION = 8708
    const val ABSTRACTION_THOUGHT = 8709
    const val ABSTRACTION_SENSATION = 8710
    const val ABSTRACTION_PURPOSE = 8711
    const val ABSTRACTION_CATEGORY = 8712
    const val ABSTRACTION_PERCEPTION = 8713
    const val ROM_OF_OPTIMALITY = 8800
    const val EXPERIMENTAL_GENE_THERAPY = 8807
    const val SELF_DEFENSE_TRAINING = 8809
    const val CONFIDENCE_BUILDING_HUG = 8811
    const val ROBIN_EGG = 8833
    const val WAX_GLOB = 9310
    const val X = 9543
    const val O = 9544
    const val MAGNIFICENT_OYSTER_EGG = 9828
    const val BRILLIANT_OYSTER_EGG = 9829
    const val GLISTENING_OYSTER_EGG = 9830
    const val SCINTILLATING_OYSTER_EGG = 9831
    const val PEARLESCENT_OYSTER_EGG = 9832
    const val LUSTROUS_OYSTER_EGG = 9833
    const val GLEAMING_OYSTER_EGG = 9834
    const val BOOMBOX = 9919
    const val GUIDE_TO_SAFARI = 9921
    const val GLITCH_ITEM = 10207
    const val LAW_OF_AVERAGES = 10325
    const val UNIVERSAL_SEASONING = 10640
    const val GREEN_MARSHMALLOW = 10746
    const val MARSHMALLOW_BOMB = 10747
    const val SHADOW_SAUSAGE = 11135
    const val SHADOW_SKIN = 11136
    const val SHADOW_FLAME = 11137
    const val SHADOW_BREAD = 11138
    const val SHADOW_ICE = 11139
    const val SHADOW_FLUID = 11140
    const val SHADOW_GLASS = 11141
    const val SHADOW_BRICK = 11142
    const val SHADOW_SINEW = 11143
    const val SHADOW_VENOM = 11144
    const val SHADOW_NECTAR = 11145
    const val SHADOW_STICK = 11146
    const val BOOK_OF_IRONY = 11867
    const val PSYCHOANALYTIC_JAR = 6055
    const val TALES_OF_SPELUNKING = 8063
    const val DODECAGRAM = 479
    const val BUTTERKNIFE = 481
}
